...
- **Focus à l'ouverture** : ouvrir le panneau place automatiquement le focus dans la zone d'écriture (idem au changement de projet actif).
- **Validation clavier** : `Ctrl+A` puis `Ctrl+C` ou `Ctrl+X` — copier ou couper **l'intégralité** du texte (sélection couvrant tout le contenu) archive le message et vide la zone, comme le bouton « Copier ». En plus, la validation clavier **ferme le panneau et rend le focus au terminal actif** (le champ de saisie de claude code), prêt pour `Ctrl+V`. Une copie partielle reste une copie ordinaire. Remplace l'ancien `Ctrl+Entrée`, retiré.
